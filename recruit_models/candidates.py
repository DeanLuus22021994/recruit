...
class Candidate(models.Model):
    ...
    education = models.CharField(max_length=25, blank=True, choices=EDUCATION_CHOICES)
    education_major = models.CharField(max_length=250, blank=True)
    current_location = CountryField(blank=True)
    image = models.ImageField(upload_to="candidate/%Y/%m/%d")
    thumb = models.ImageField(upload_to="candidate/%Y/%m/%d", blank=True)
...
class CandidateRequirements(models.Model):
    ...
    employer_type = models.CharField(
        max_length=25, blank=True, choices=EMPLOYER_TYPE_CHOICES
    )
